...
function timeCompare(time1, time2) {
  var unixHours = 0.5 * 60 * 60; // 0.5 hour in seconds
            ...
            pulls.map(function (pull, i) {
              if (pull.name == activeBoss) {
                var bossHp = pull.bossPercentage,
                  bossHpRounded;
                if (pull.kill == true) {
                  // check if this is a kill, because % might not be accurate
                  bossHpRounded = "0";
                } else {
          ...
          _loop2 = /*#__PURE__*/_regeneratorRuntime().mark(function _loop2() {
            ...
            return _regeneratorRuntime().wrap(function _loop2$(_context) {
              while (1) switch (_context.prev = _context.next) {
                ...
                case 5:
                  data = _context.sent;
                  innerFightData = data.data.reportData.reports.data; // console.log(data);
...
var buildChart = fetchMetaData().then(function (result) {
  ...
  for (var _x = 0; _x <= mapLowestHP.length; _x++) {
    // A value of 100 fucks up the logic for the 'day' bars, so just throw them out
    if (mapLowestHP[_x] == 100) {
      mapLowestHP[_x] = mapLowestHP[_x - 1];
    }
    ...
    if (mapLowestHP[_x] < mapLowestHP[_x + 1]) {
      mapLowestHP[_x + 1] = mapLowestHP[_x];
    }
...
function updateRefreshTime() {
  ...
  var dateFile = '/js/fetchDate.txt'; // provide file location
  fetch(dateFile).then(function (response) {
    return response.text();
  }).then(function (data) {
    elem.innerHTML = "Updated at ".concat(data);
  });
}
updateRefreshTime();
