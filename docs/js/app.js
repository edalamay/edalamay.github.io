(function (factory) {
	...
	// header video
	//===========================================
	const ytParent = document.querySelector('.section--banner .background'),
	...
	async function getStreams() {
	  ...
	  fetch(endpoint, {
	    headers
	  }).then(res => res.json()).then(data => renderStreams(data));
	}
	  ...
	  if (streams.length > 0) {
	    streams.forEach((stream, i) => {
	      ...
	      if (game_name == 'World of Warcraft') {
	        let hdThumbnail = thumbnail.replace("{width}", "533").replace("{height}", "300");
	        ...
	        streamerBlock.innerHTML = `
						...
						<p class="streamer--views">
							${viewer_count.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")}
						</p>
					`;
	    ...
	    // create stream
	    var activeStreamers = document.querySelectorAll('.streamer'),
